...
def source_stem(value):
    return Path(str(value or "untitled")).name.replace(".html", "").replace(".log", "").replace(".txt", "").replace(".g", "")
...
def build_gap_script(stem):
    txt_path = ROOT / f"{stem}.txt"
    log_path = ROOT / f"{stem}.log"
    if not txt_path.exists():
        raise FileNotFoundError(f"No input file {txt_path.name}")
    step2_path = SOURCE / "Step2Core.g"
    return f'''# GAP/QPA script generated by AR Quiver: Run with GAP
#
# Usage:
#   1. Copy this whole file into GAP, or save it as {stem}_run_with_gap.g.
#   2. Make sure the QPA package is installed and GAP can read Step2Core.g.
#   3. After it runs, use these variables:
#        Q       original quiver
#        kQ      path algebra over Rationals
#        rel     relations
#        A       bound quiver algebra kQ / rel
#        M[i]    indecomposable modules in the computed AR component
...
input_txt_path := "{gap_string(txt_path.as_posix())}";;
output_log_path := "{gap_string(log_path.as_posix())}";;
Read("{gap_string(step2_path.as_posix())}");;

# Step2Core.g parses input_txt_path and constructs Q, kQ, rel, A, P, I, S.
# RunARQuiverComputation computes the AR data and stores the computed modules in V.
RunARQuiverComputation();;

M := V;;

Print("\\nGenerated variables ready.\\n");
Print("Algebra: A = kQ / rel\\n");
Print("Modules: M[1]..M[", Length(M), "]\\n");
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            ...
            return
        if path == "/api/gap/script":
            query = parse_qs(urlparse(self.path).query)
            stem = source_stem((query.get("source") or ["untitled"])[0])
            data = build_gap_script(stem).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "text/plain; charset=utf-8")
            self.send_header("Content-Disposition", f'inline; filename="{stem}_run_with_gap.g"')
